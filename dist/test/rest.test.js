...
async function post(path, body, extra = {}) {
    return await app.request(`https://example.org${path}`, { method: "POST", headers: { ...JSON_HEADERS, ...extra }, body: JSON.stringify(body) }, env);
}
...
describe("REST maps", () => {
    ...
    it("GET /v1/maps/:code returns the compiled map; unknown codes 404", async () => {
        ...
        expect((await missing.json()).error.code).toBe("map_not_found");
    });
});
