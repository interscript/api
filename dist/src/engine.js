...
function ensureEngine(assets) {
    if (configuredFor === assets)
        return;
    const strategy = (code) => loadMap(assets, code);
    configure({ strategies: [strategy] });
    configuredFor = assets;
}
export async function transliterate(assets, systemCode, input) {
    ensureEngine(assets);
...
export async function detect(assets, input, output) {
    ensureEngine(assets);
    const candidates = [];
