...
import { API_VERSION, detectResolver, setAssets, transliterateResolver, } from "./resolvers.js";
...
rest.get("/v1/maps", (c) => c.json({ count: bundledSystemCodes().length, maps: bundledSystemCodes() }));
rest.get("/v1/maps/:code", async (c) => {
    const assets = c.env?.ASSETS;
    if (!assets)
        return errorResponse(503, "assets_missing", "ASSETS binding not configured");
    const code = c.req.param("code");
    if (!bundledSystemCodes().includes(code)) {
...
rest.get("/v1/models/:id", (c) => {
    ...
    if (!model)
        return errorResponse(404, "model_not_found", `Couldn't locate ${c.req.param("id")}`);
    return c.json(model);
});
rest.post("/v1/infer", async (c) => {
    ...
    const { ML_ENDPOINT, ML_TOKEN } = (c.env ?? {});
    if (!ML_ENDPOINT || !ML_TOKEN) {
        return errorResponse(503, "inference_unconfigured", "ML_ENDPOINT/ML_TOKEN are not set on this deployment");
    }
