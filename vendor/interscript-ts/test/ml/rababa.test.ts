...
describe("Rababa haraqat constants", () => {
  ...
  it("ID_TO_HARAAQAT includes pad prefix and matches model vocab", () => {
    // The trained rababa model's target vocab is [@pad] + haraqat + [unused start slot].
    // Index 0 = pad ("P"), indices 1..15 = haraqat in ALL_POSSIBLE_HARAQAT order,
    // index 16 = unused start slot (decodes to "" at inference time).
    const haraqatKeys = Object.keys(ALL_POSSIBLE_HARAQAT)
    expect(ID_TO_HARAAQAT[0]).toBe("P")
    expect(ID_TO_HARAAQAT.slice(1, 1 + haraqatKeys.length)).toEqual(haraqatKeys)
    expect(ID_TO_HARAAQAT.length).toBe(1 + haraqatKeys.length + 1)
  })
