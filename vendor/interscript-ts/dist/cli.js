...
import { configure, reset, transliterate, transliterateAsync, detect, httpStrategy, } from "./index.js";
import { filesystemStrategy } from "./loaders.node.js";
function buildStrategies(opts) {
    if (opts.http) {
        const httpOpts = { baseUrl: opts.http };
        if (!opts.noCache)
            httpOpts.cacheKeyPrefix = "isx-cli:";
...
function loadCatalogue(mapsDir) {
    const out = [];
...
async function cmdTransliterate(args, opts) {
...
async function cmdBatch(args, opts) {
    ...
    for (const input of lines) {
        try {
            ...
            rows.push({ input, output: "", error: e.message });
        }
    }
    const fmtRow = (r) => values.csv
...
function cmdList(args, opts) {
    ...
    if (!existsSync(mapsDir)) {
        process.stderr.write(`Error: --maps-dir required (or run from a directory containing ./maps).\n`);
        return 2;
    }
    ...
    const filtered = entries.filter((e) => {
        if (!e.metadata)
            return true;
        if (values.authority && e.metadata.authority_id !== values.authority)
            return false;
        if (values["source-script"] && e.metadata.source_script !== values["source-script"])
            return false;
        if (values["destination-script"] && e.metadata.destination_script !== values["destination-script"])
            return false;
        return true;
    });
...
async function cmdDetect(args, opts) {
    const { values, positionals } = parseArgs({
        args,
        options: {
