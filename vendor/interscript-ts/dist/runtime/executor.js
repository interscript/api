...
import { compileParallelTree, parallelReplaceTree, parallelMegaregexp, downcase, upcase, titleCase, separate, compose, decompose, } from "../stdlib.js";
...
const executors = {
    ...
    run: (rule, ctx) => {
        ...
        if (!target)
            throw new MapLogicError(`Stage not found: ${rule.stage}`);
    ...
    funcall: (rule, ctx) => {
        ...
        if (!fn)
            throw new MapLogicError(`Unknown function: ${rule.name}`);
    ...
    parallel: (rule, ctx) => {
        ...
        const treeCompatible = (r) => !r.before && !r.after && !r.notBefore && !r.notAfter && expandFromLiterals(r.from, ctx) !== null;
        ...
        if (!anyConstrained) {
            ...
            for (const r of subRules) {
                if (r.to && r.to.kind === "funcall_inline")
                    continue;
            ...
            const sorted = subRules
                ...
                rule: r,
                idx,
                len: maxLengthOfItem(r.from, ctx) +
            ...
            const megaRules = sorted.map(({ rule: r }) => {
                ...
                const replaceFn = typeof replacement === "string"
                    ? (match, _groups) => resolveTemplate(replacement, match, _groups)
                    : replacement;
...
function executeSubRule(rule, ctx) {
    if (!rule.from)
        throw new MapLogicError("Sub rule missing 'from'");
    ...
    if (notBefore)
        patternParts.push(`(?<!${notBefore})`);
    patternParts.push(from.re);
    if (after)
        patternParts.push(`(?=${after})`);
    if (notAfter)
        patternParts.push(`(?!${notAfter})`);
    let re;
    try {
    ...
    ctx.current = ctx.current.replace(re, typeof replacement === "string"
        ? (() => {
            // Use function to avoid $' $` $$ special-meaning bugs in
            // String.replace replacement strings.
            const tmpl = replacement;
            return (match, ...args) => resolveTemplate(tmpl, match, args);
        })()
        : replacement);
}
/**
 ...
 */
function resolveTemplate(template, match, groups) {
 ...
 */
function buildReplacement(to, ctx) {
    if (!to)
        return "";
    if (to.kind === "funcall_inline") {
        const fn = resolveFunction(ctx, to.name);
        if (!fn)
            throw new MapLogicError(`Unknown inline function: ${to.name}`);
...
export function executeRule(rule, ctx) {
    const executor = executors[rule.kind];
    executor(rule, ctx);
}
