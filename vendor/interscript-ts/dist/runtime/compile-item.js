 ...
 */
export function compileItem(item, ctx) {
    switch (item.kind) {
        case "string":
 ...
 */
export function compileToLiteral(item, ctx) {
    switch (item.kind) {
        case "string":
            return item.value;
        case "alias": {
            const stdlib = STDLIB_ALIASES[item.name];
            if (stdlib)
                return stdlib.literal === "" ? null : stdlib.literal;
            const resolved = ctx.resolveAlias(item.name);
            if (!resolved)
                return null;
 ...
 */
export function expandFromLiterals(item, ctx) {
    switch (item.kind) {
        case "string":
            return [item.value];
        case "alias": {
 ...
 * `none` is 0.
 */
export function maxLengthOfItem(item, ctx) {
    switch (item.kind) {
        case "string":
 ...
 */
const STDLIB_ALIASES = Object.freeze({
...
//# sourceMappingURL=compile-item.js.map