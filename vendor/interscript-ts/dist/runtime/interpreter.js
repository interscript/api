...
export function registerAsyncFunction(name) {
 ...
 */
function containsAsyncFuncalls(rules) {
    for (const r of rules) {
        if (r.kind === "funcall" && ASYNC_FUNCTIONS.has(r.name))
            return true;
        if (r.kind === "parallel" || r.kind === "sequential") {
            for (const inner of r.rules) {
                if (inner.kind === "funcall" && ASYNC_FUNCTIONS.has(inner.name))
                    return true;
            }
        }
    }
 ...
 * for those.
 */
export function executeStage(map, stageName, input, loader) {
    const stage = map.stages.find((s) => s.name === stageName);
    if (!stage) {
        return input;
    }
    if (containsAsyncFuncalls(stage.rules)) {
        throw new Error(`Stage "${stageName}" contains ML function calls (rababa/secryst). ` +
 ...
 */
export async function executeStageAsync(map, stageName, input, loader) {
    const stage = map.stages.find((s) => s.name === stageName);
    if (!stage) {
        return input;
    }
 ...
 */
async function executeRuleAsync(rule, ctx) {
...
async function executeFuncallAsync(rule, ctx) {
    ...
    if (ASYNC_FUNCTIONS.has(rule.name)) {
        ...
        const modelId = (rule.kwargs?.config ?? rule.kwargs?.model ?? "default");
        const model = await loadModel({ kind: rule.name, id: modelId });
        ctx.current = await model.transform(ctx.current);
        return;
    }
    // Fall back to the sync funcall executor for all other functions.
    executeRule({ kind: "funcall", name: rule.name, kwargs: rule.kwargs }, ctx);
}
