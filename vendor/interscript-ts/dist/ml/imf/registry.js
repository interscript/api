...
export const DEFAULT_INDEX_URL = "https://raw.githubusercontent.com/interscript/ml-models/main/models.yaml";
...
async function fetchIndex(source) {
    ...
    if (raw.version !== 1)
        throw new RegistryError("index must have version: 1");
    const entries = {};
...
export async function resolve(modelId, indexUrl) {
    ...
    if (fs?.existsSync(target)) {
        ...
        if ((await sha256Hex(cached)) === entry.sha256)
            return { bytes: cached, path: target };
    }
    ...
    if (actual !== entry.sha256) {
        throw new RegistryError(`downloaded ${entry.filename} sha256 mismatch: got ${actual}, index says ${entry.sha256}`);
    }
    if (fs) {
