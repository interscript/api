...
export function parseManifest(zipBytes) {
    const files = unzipSync(zipBytes);
    const metaBytes = files["metadata.yaml"];
    if (!metaBytes)
        throw new IMFError("missing metadata.yaml");
    const raw = loadYaml(new TextDecoder().decode(metaBytes));
    if (raw["format"] !== "imf-v1")
        throw new IMFError(`unsupported format: ${String(raw["format"])}`);
...
export async function verifyAndRead(zipBytes) {
    ...
    for (const [name, bytes] of Object.entries(files)) {
        ...
        if (!recorded)
            throw new IMFError(`${name} is not covered by metadata sha256`);
