 ...
 */
function buildPivotMap(dOriginal, dDiacritized) {
    const out = [];
    let idxOri = 0;
 ...
 * `reconcile_strings`.
 */
export function reconcileStrings(strOriginal, strDiacritized) {
    ...
    for (const { idxDia, idxOri } of pivots) {
        if (ptOri < idxOri) {
            for (let i = ptOri; i < idxOri; i++)
                out += dOriginal[i];
        }
        if (ptDia < idxDia) {
            for (let i = ptDia; i < idxDia; i++)
                out += dDiacritized[i];
        }
    ...
    for (let i = ptDia; i < dDiacritized.length; i++)
        out += dDiacritized[i];
    // Trailing original chars
    for (let i = ptOri; i < dOriginal.length; i++)
        out += dOriginal[i];
    return out;
}
