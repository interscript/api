...
import { artifactUrls, resolveManifestEntry, sidecarFilenames, } from "./manifest.js";
/**
 ...
 */
export async function provisionModel(ref, opts = {}) {
    ...
    const artifacts = {};
    await Promise.all(sidecars.map(async ({ filename, url }) => {
        try {
    ...
    const sessionOpts = { webgpu };
    if (format === "tflite") {
        sessionOpts.runtime = "litert";
        if (opts.litertAccelerator !== undefined)
            sessionOpts.litertAccelerator = opts.litertAccelerator;
        if (opts.inputNames !== undefined)
            sessionOpts.inputNames = opts.inputNames;
        if (opts.outputNames !== undefined)
            sessionOpts.outputNames = opts.outputNames;
    }
    const session = await createSession(modelBuffer, sessionOpts);
    return { session, artifacts };
}
async function resolveModelUrl(ref, variant, format) {
    const entry = await resolveManifestEntry(ref.kind, ref.id);
    if (!entry) {
        throw new Error(`No manifest entry for kind=${ref.kind} id=${ref.id}. ` +
...
async function resolveSidecarUrls(ref, variant, format) {
